<x-app-layout>
    ...
    <div class="flex justify-center py-10">
        <div
            ...
            <form class="space-y-6" action="{{route('my_books.update',$book)}}" method="post"
                ...
                <div class="block mb-2 mx-auto font-medium text-gray-900 dark:text-white">
                    <img class="h-auto max-w-xs w-1/2 mx-auto" src="{{$book->full_image_url}}" alt="{{$book->name}}">
                </div>
        @if($book->image_url)
                <div class="flex w-full justify-center" x-data="{
                    ...
                    <button type="button" @click="deleteImage(`{{route('my_books.delete_image',$book)}}`)"
                        class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2">Remove
                    </button>
                </div>
@endauth
                <div>
                ...
                <div class="space-y-2">
                    ...
                    <textarea id="message" rows="4"
                        ...
                        placeholder="Summary..." name="summary" required>{{$book->summary}}</textarea>
                    @error('summary')
                    <p class="text-red-600">
                        {{$message}}
                    </p>
    ...
    <input name="is_public" @checked($book->is_public) id="default-checkbox" type="checkbox" value="1" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
    <label for="default-checkbox" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300" name="is_public">mark as public</label>
</div>
